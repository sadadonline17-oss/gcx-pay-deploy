...
import { Heart, Copy, ExternalLink, Plus, DollarSign, FileText, Hash } from "lucide-react";
import { useToast } from "@/hooks/use-toast";
import { useCreateLink } from "@/hooks/useSupabase";
import BackButton from "@/components/BackButton";
import BottomNav from "@/components/BottomNav";

const CreateHealthPaymentLink = () => {
  ...
  const serviceTypes = [
    ...
    { value: "eye", label: "طب العيون" },
    { value: "physiotherapy", label: "علاج طبيعي" },
    { value: "mental", label: "صحة نفسية" },
  ];
  ...
  const handleGenerateLink = async (e: React.FormEvent) => {
    ...
    try {
      const link = await createLink.mutateAsync({
        ...
        payload: {
          service_name: formData.serviceName || "خدمة صحية",
      ...
      const paymentUrl = `${productionDomain}/pay/${link.id}?company=health&currency=${currencyInfo.code}&amount=${formData.amount}&method=${formData.paymentMethod}`;
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-red-50 via-pink-50 to-white py-6" dir="rtl">
      <div className="container mx-auto px-4 max-w-3xl">
        ...
        <div className="text-center mb-8">
          <div className="flex items-center justify-center gap-3 mb-4">
            <div className="w-16 h-16 bg-gradient-to-br from-red-500 to-pink-500 rounded-2xl flex items-center justify-center shadow-xl">
              <Heart className="w-8 h-8 text-white" />
            </div>
          </div>
          <h1 className="text-4xl font-black mb-3 bg-gradient-to-r from-red-600 via-pink-600 to-red-600 bg-clip-text text-transparent">
            مولد روابط دفع الخدمات الصحية
          </h1>
        ...
        {!generatedLink ? (
          <Card className="p-8 shadow-2xl border-2 border-red-100">
            <form onSubmit={handleGenerateLink} className="space-y-6">
              <div>
                ...
                <Input
                  id="serviceName"
                  placeholder="مثال: استشارة طبية - د. أحمد محمد"
              ...
              <div>
                ...
                <Textarea
                  id="description"
                  placeholder="أضف وصف تفصيلي للخدمة..."
              ...
              <Button
                ...
                className="w-full h-14 text-lg font-bold bg-gradient-to-r from-red-600 to-pink-600 hover:from-red-700 hover:to-pink-700 shadow-xl"
              >
...
export default CreateHealthPaymentLink;
